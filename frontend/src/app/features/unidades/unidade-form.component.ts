...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ editando ? 'Editar' : 'Nova' }} unidade acadêmica</h2>
...
export class UnidadeFormComponent {
  ...
  private ref = inject(MatDialogRef<UnidadeFormComponent>);
  private data = inject<UnidadeAcademica | null>(MAT_DIALOG_DATA);
  ...
  protected salvar(): void {
    ...
    req.subscribe({
      next: () => {
        this.notify.success(this.editando ? 'Unidade atualizada.' : 'Unidade criada.');
